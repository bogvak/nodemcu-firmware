...
## ws2812.buffer:set()
Set the value or values starting from the given position
...
- `string` should contains no more than `(numberOfLeds-index)*bytesPerLed` bytes
- `ws2812.buffer` should be the same `bytesPerLed` as destination buffer and has size no more than `numberOfLeds-index`
...
```lua
buffer = ws2812.newBuffer(32, 3)
buffer_to_copy = ws2812.newBuffer(10, 3)
buffer:set(1, buffer_to_copy) -- copy buffer_to_copy to buffer starting from 1st LED
```
